    ...
    for i in range(len(image_names)):
        ...
        with lock:
            with open(args.save_file, 'a') as f:
                f.write(f"{image_names[i].strip()} \"{response.strip()}\"\n")
            curr_time = time.time()
            time_to_finish = (curr_time - begin_time) / (i + 1) * (len(image_names) - i)
            print(f"proc {num} {i}/{len(image_names)} estimate time to finish {time_to_finish / 60:.2f} mins. Result: {response.strip()}")
